...
class MLP:
	...
	def fit(self,X):
		...
		while i < self.num_epochs:
			...
			z_s,a_s = self.forwardandbackprop(X[batch_index-batch_size:batch_index])
			der_b,der_w,e = self.backprop(a_s,z_s,X[batch_index-batch_size:batch_index])

			for j in range(self.n_layers-1):
				self.biases[j] -= (e/100)*der_b[j]
	...
	def backprop(self,a_s,z_s,y):
		...
		for i in range(2,self.n_layers):
			...
			if i-1 == self.n_layers//2:
				z_bar = np.sum(a_s[-i],axis=1)/y.shape[0]
				KL_grad = self.lamda*((-self.p/z_bar)+((1-self.p)/(1-z_bar)))
				delta = np.multiply(np.dot(delta,self.weights[-i+1].T)+np.matrix(KL_grad).T,sigmoidDer(z_s[-i]))
			else:
...
if __name__ == '__main__':
	...
	n_layers_sizes = [64,32,64]
	mlp = MLP(n_layers_sizes,learning_rate=0.002,num_epochs=2e5).fit(x_train)
